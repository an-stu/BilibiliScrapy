...
Style: AcplayDefault, %(font_name)s, %(font_size)s, &H00FFFFFF, &H00FFFFFF, &H00000000, &H00000000, 1, 0, 0, 0, 100, 100, 0.00, 0.00, 1, 1, 0, 2, 20, 20, 20, 0
[Events]
...
def convert(input,resolution,font_name,font_size,line_count,bottom_margin,shift,limit):
    XML_NODE_RE = re.compile('<d p="([^"]*)">([^<]*)</d>')
    nico_subtitles = []
    nico_subtitle_lines = XML_NODE_RE.findall(input)
    nico_subtitle_lines = nico_subtitle_lines[:limit]
